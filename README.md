...
> read and write)**, **M6 (Notifications and indications)**, **M7 (Protocol parsers)**,
> **M8 (Session recording)** and **M9 (Export)** are implemented. See
...
characteristics, subscribes to notifications, reads and writes values, records sessions,
exports them as JSON or CSV, and visualizes live device data. It is built to make invisible
Bluetooth activity visible: scan events, connection transitions, raw bytes, parsed values,
RSSI, errors, and timing.
...
- The Bluetooth bridge surface is `apps/mobile/src/native/specs/NativeBeaconBluetooth.ts`, a
  codegen spec using primitives, plain objects, promises and typed event emitters. It is the
  only way to the radio; the one other module, `NativeBeaconExport.ts`, does not touch it.
...
- Export adds no bridge segment either; it adds a second Turbo Module, `BeaconExport`, with
  three calls (`writeTemporaryFile`, `shareFile`, `clearTemporaryFiles`) and its own
  `ExportErrorCode` union, because writing a file and presenting a share sheet are not
  Bluetooth operations and folding them into `BleErrorCode` would make that union mean two
  things. `createNativeExportClient` wraps it the same way `createNativeBleClient` wraps the
  radio. See [ADR 0010](docs/ADR/0010-export-formats-and-share-pipeline.md).
...
sketches it), grows in place while the session is still recording, and can delete or export an
ended session. See [ADR 0009](docs/ADR/0009-session-persistence.md).

## Export

`packages/session-export`, `apps/mobile/src/features/export/` and the second Turbo Module in
`apps/mobile/ios/BeaconExport` and `com/beacon/export`: an ended session can leave the device
as JSON or CSV through the platform share sheet, and the two formats have different jobs
(PROJECT.md 21). JSON is the record: a versioned document (`version`, `generator`,
`exportedAt`, `session`, `events`) carrying every field the repository holds, with its keys in
a canonical order so the output is byte-stable — two exports of a session are the same file,
and re-exporting a parsed document reproduces it, which is what lets a diff, a checksum or a
signature over an export mean anything. `parseJsonExport` reads one back through the same zod
schemas the repository validates its own rows with, and the tests hold the round trip; the App
test reads back the document the app produced from a session it recorded through the
navigator. CSV is the report: a summary block naming the session, a blank line, then one row
per event with short UUIDs, hex bytes and a readable detail for the kinds that carry none,
quoted per RFC 4180, with a leading quote on any value a spreadsheet would treat as a formula
so nothing that arrived over the air can execute when the file is opened.

Writing the file and presenting the sheet are a second Turbo Module, `BeaconExport`, with
three calls (`writeTemporaryFile`, `shareFile`, `clearTemporaryFiles`) and its own error
codes, deliberately separate from `BeaconBluetooth` and `BleErrorCode`: neither is a Bluetooth
operation. Both platforms only ever share a file the module itself wrote — iOS compares
resolved path components, Android compares canonical paths, and a file name from JavaScript is
reduced to its last path component — and Android hands the file out through a `FileProvider`
scoped to the one `beacon-exports` cache subdirectory, since the chooser grants the app the
person picks read access to whatever URI it is given. The completion signal differs by
platform and the UI is honest about it: iOS reports a dismissal accurately through
`UIActivityViewController`, while Android resolves true once the chooser starts, because
`Intent.createChooser` finishes as soon as a target is picked and almost no receiving app
reports a result — so the app says a file "was handed to the share sheet", never that it was
delivered. Export is explicitly user-initiated (PROJECT.md 35), one at a time, and a session
that is still recording cannot be exported, because the file would hold only part of it; the
events are read from the repository rather than from the screen, so the document is always the
whole session. The controls say that an export carries the device identifier and every
recorded value before anyone shares one. See
[ADR 0010](docs/ADR/0010-export-formats-and-share-pipeline.md).
...
| Bridge     | Two Turbo Native Modules with codegen (`BeaconBluetooth`, `BeaconExport`), typed event emitters                  |
...
| Packages   | `@beacon/ble-contracts`, `@beacon/validation`, `@beacon/protocol-parsers`, `@beacon/session-export`              |
...
│   ├── ios/BeaconBluetooth/     Swift CoreBluetooth core + the Objective-C++ Turbo Module shim
│   ├── ios/BeaconExport/        Swift export file store, share sheet presenter + its shim
│   ├── android/.../com/beacon/bluetooth/  Kotlin BLE controller, scanner, connections, mappers
│   ├── android/.../com/beacon/export/     Kotlin export file store, FileProvider share, package registration
│   └── src/
...
│       ├── features/export/     export machine, its presentation text, the session export hook and controls
│       ├── storage/             SqlDatabase interface, op-sqlite binding, versioned migrations
│       ├── mock/                scripted mock BLE client (runtime option)
│       ├── native/              Turbo Module specs (BeaconBluetooth, BeaconExport) + validated client wrappers
│       └── theme/
│   └── tests/                   App integration tests, FakeBleClient, FakeExportClient, sql.js-backed SqlDatabase for Jest
├── packages/
...
│   └── session-export/          JSON and CSV serializers, canonical key order, export file names, export error codes
...
- [ADR 0010](docs/ADR/0010-export-formats-and-share-pipeline.md): two export formats with different jobs, full-fidelity JSON with canonical key order so the document is byte-stable and round trips through the repository's own schemas, and a flattened human-readable CSV with RFC 4180 quoting and formula neutralisation; a second Turbo Module for the file and the share sheet with its own error codes, kept off the BLE module; only files the module wrote are shared, scoped on Android by a `FileProvider`; exports user-initiated, one at a time, whole-session and refused while recording; the completion boolean reported honestly per platform.
...
| M9 Export                          | Implemented, CI-validated, **no hardware validation** | `@beacon/session-export`: a full-fidelity JSON document (`version`, `generator`, `exportedAt`, `session`, `events`) with canonical key ordering so the output is byte-stable and re-exporting a parsed document reproduces the file, read back through the repository's own zod schemas; a human-readable CSV with a session summary block, one row per event, short UUIDs, hex bytes, RFC 4180 quoting and formula neutralisation; a second Turbo Module, `BeaconExport` (`writeTemporaryFile`, `shareFile`, `clearTemporaryFiles`), with its own error codes kept out of `BleErrorCode`; iOS `UIActivityViewController` and Android `Intent.createChooser` behind a `FileProvider` scoped to one cache subdirectory, both sharing only files the module itself wrote; an export machine that is user-initiated, one at a time, whole-session and refused while recording, with Export JSON / Export CSV, a status line and the device-identifier note on the session detail screen. 44 tests in the export package, bridge wrapper tests against a fake spec, reducer and presentation tests, five App tests driving the whole flow through a fake export client, 9 XCTest cases and 11 JUnit cases for the two native file stores. The native modules are compiled by CI's Android and iOS jobs only; no person has yet presented a share sheet, or written an export file, on a device. |
| M10 and later                      | Not started                                           |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
