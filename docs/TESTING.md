...
`packages/session-export` is its own Jest project (`session-export`, node environment, listed
in the root `projects` array alongside `ble-contracts`, `validation` and `protocol-parsers`),
so its 44 tests run under `yarn test` and in the `js` CI job with the rest. M9 also adds the
second Turbo Module, so there are new XCTest and JUnit suites for the first time since M6 —
both for the file store on each platform, which is written free of UIKit and of Android
framework types precisely so the containment rules can be asserted without a device. What
cannot be tested is everything that needs a live window: `ShareSheetPresenter` and the Android
chooser are checked by hand, and M9 has not had that.

The App tests drive the whole flow through `tests/fakes/FakeExportClient.ts`, which implements
`ExportClient` and keeps the write and the share pending until the test settles them, the same
way `FakeBleClient` does for native calls. Because the fake keeps the document, those tests
assert on the real file the app produced — one of them reads it back with `parseJsonExport`,
which proves the app can read what it writes over a session it recorded through the navigator
rather than over a fixture.

| Suite                                                                  | Covers                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| ---------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `packages/session-export/src/__tests__/json.test.ts`                   | Session and every event kind preserved through `toJsonExport` → `parseJsonExport`; re-exporting a parsed document reproduces the file byte for byte, and a session handed over with its keys in another order writes the same bytes; version, generator and export instant, defaulting to now; events copied so a later mutation cannot change the document; the trailing newline, the default indent and the compact form; an open session round tripped; parse failures for non-JSON, a non-object, an unsupported version, a schema-rejected session, a named bad event, a missing events array |
| `packages/session-export/src/__tests__/csv.test.ts`                    | The summary block, the blank line and the table header in order; one row per event and every row terminated including the last; SIG UUIDs shortened, a vendor UUID kept, `fullUuids` honoured; a readable detail for the kinds that carry no bytes; empty byte columns rather than zeros for those kinds and a real zero for an empty write; a custom newline; RFC 4180 quoting for a comma, an embedded quote and a newline; a formula-looking value neutralised; the session summary on its own, including a session still recording                                                             |
| `packages/session-export/src/__tests__/fileName.test.ts`               | The device name, start instant and format in the name; the device id when there is no name; characters a file system would reject replaced; the fallback when nothing usable is left; a very long name capped; each format mapped to its media type                                                                                                                                                                                                                                                                                                                                                |
| `packages/session-export/src/__tests__/errors.test.ts`                 | An `ExportError` passed through unchanged; the code read off a native rejection; the fallback when the code is not one of ours and when the caller supplies its own; a thrown non-error; only the module's own codes recognised                                                                                                                                                                                                                                                                                                                                                                    |
| `apps/mobile/src/native/__tests__/createNativeExportClient.test.ts`    | Against a hand-written fake of the export spec: the name and contents forwarded and the path returned; a native rejection turned into an `ExportError` keeping its code, and an uncoded one defaulted per call; a module that returns no path or does not say whether the share completed rejected; a completed share and a dismissal reported distinctly; the media type passed through; a nonsense clear count resolved as zero                                                                                                                                                                  |
| `apps/mobile/src/features/export/__tests__/exportReducer.test.ts`      | `idle → preparing → sharing → idle` with the outcome kept; a dismissal recorded distinctly from a completed share; a second request ignored while one runs; the format kept on a failure, a failure while the sheet is open, and a failure or a settled share ignored when nothing is running; the previous outcome replaced on the next export                                                                                                                                                                                                                                                    |
| `apps/mobile/src/features/export/__tests__/exportPresentation.test.ts` | Both formats offered while idle; progress on the running format with the other disabled; the sharing line; nothing said before the first export; a completed share claiming only that the file reached the sheet; the dismissal and failure lines; the format names; the device-identifier note                                                                                                                                                                                                                                                                                                    |
| `apps/mobile/tests/App.test.tsx` ("App (session export)")              | Through the real navigator, the fake client, `InMemorySessionRepository` and `FakeExportClient`: a JSON document of the whole recorded session written, read back with `parseJsonExport` and handed to the sheet as `application/json` with both buttons disabled in between; the CSV with its summary block, header and one row per event as `text/csv`; a dismissal reported without claiming delivery; a failed export explained and retried; a session still recording refusing to export at all                                                                                               |
| `apps/mobile/ios/BeaconBluetoothTests/ExportFileStoreTests.swift`      | Nine cases: the directory created on first write and the contents round tripped as UTF-8; the same name written twice replacing the file; a name with directory parts reduced to its last component and one that reduces to nothing rejected; `contains` accepting a file the store wrote, rejecting a path outside the directory and rejecting a traversal back out; `clear` removing files, reporting the count and being safe to call twice or before the directory exists                                                                                                                      |
| `apps/mobile/android/app/src/test/.../export/ExportFileStoreTest.kt`   | Eleven cases: the same behaviour on the JVM, with a traversing and an absolute file name both reduced to their last component, `contains` rejecting a blank path as well as one outside the directory or walking back out, and clearing before the first export reporting nothing removed                                                                                                                                                                                                                                                                                                          |

Run the export suites alone with `yarn jest packages/session-export
apps/mobile/src/features/export createNativeExportClient` from the repo root, or the package on
its own with `yarn jest --selectProjects session-export`; `yarn test` runs them with
everything else. The native suites run with the existing commands: the Swift cases live in the
`BeaconBluetoothTests` target (there is one test target, not one per module) and the Kotlin
ones in `:app:testDebugUnitTest`.
